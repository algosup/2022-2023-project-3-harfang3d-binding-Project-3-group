...
extern "C"
{
    ...
    double Vector3::v3distanceTo(Vector3 pos) {
        return sqrt((pos.y - y) * (pos.y - y) + (pos.x - x) * (pos.x - x) + (pos.z - z) * (pos.z - z));
    }
    void Vector3::v3vectorMovement(double plusx, double plusy, double plusz) {
        x += plusx;
        y += plusy;
        z += plusz;
    }

    Vector3 Vector3::v3midpoint(Vector3 pos) {
        return Vector3((x + pos.x) / 2, (y + pos.y) / 2, (z + pos.z) / 2);
    }

    double Vector3::v3percentDistance(Vector3 pos, double percentOfDistance) {
        return v3distanceTo(pos) * percentOfDistance;
    }
